...
// The WebGL scene (three.js + react-three-fiber) is a heavy, browser-only
// bundle — load it lazily and only on the client, so it never inflates the
// server-rendered payload and never loads at all when we fall back to the
// CSS scene (no WebGL / reduced motion).
...
function AnimatedHeadline() {
  ...
  return (
    <h1
      className="text-balance text-4xl font-display font-medium leading-[1.08] tracking-tight text-paper sm:text-6xl lg:text-7xl"
...
export function Hero() {
  ...
  const canRender3d = webglAvailable === true && !reducedMotion;
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-ink">
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col items-start justify-center px-6 lg:px-8">
          <motion.p
            ...
            className="mb-6 inline-flex items-center gap-2 rounded-full border border-line/80 bg-surface/60 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-accent-400 backdrop-blur"
          >
          ...
          <motion.p
            ...
            className="mt-6 max-w-xl text-balance text-lg leading-relaxed text-paper/70"
          >
            {heroContent.subcopy}
          </motion.p>
          ...
          >
            ...
            <MagneticCta href={heroContent.secondaryCta.href} variant="secondary">
          ...
          <motion.dl
            ...
            className="mt-14 flex flex-wrap gap-x-12 gap-y-6 border-t border-line/70 pt-8"
          >
        ...
        <motion.div
          ...
          className="pointer-events-none absolute inset-x-0 bottom-8 z-10 flex flex-col items-center gap-3 text-paper/50"
        >
